...
def demo(args):
    ...
    with torch.no_grad():
        for in_subfolder in tqdm(Path(args.path).iterdir()):
            out_subfolder = output_dir / in_subfolder.name
            out_subfolder.mkdir(exist_ok=True)

            images = list(in_subfolder.glob("*.png")) + list(in_subfolder.glob("*.jpg"))
            ...
            for imfile1, imfile2 in zip(images[:-1], images[1:]):
                ...
                save(flow_up, imfile1, out_subfolder)
